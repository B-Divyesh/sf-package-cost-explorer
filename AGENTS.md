# Exports-aware npm package cost explorer (install size + per-entry bundle size), fully client-side (`package-cost-explorer`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://package-cost-explorer.sociobot.in`. Artifact class: `static-web`.
