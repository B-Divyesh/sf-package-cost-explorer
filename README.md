...
- a canonical share URL and an embeddable per-report SVG badge carrying the
  selected package, resolved version, and measured gzip value.
...
`npm run test:live` runs a clean-profile check against production: it verifies
that first install has no false update toast, a missing package has no expected
404 console error, the exact badge URL generated by the UI contains the report
values, and the manifest has its interoperable MIME type.
...
Deploy `dist/` with the adjacent `api/` Azure Functions Node v4 worker as a
Standard Azure Static Web App. The checked-in Static Web Apps configuration
rewrites `/badge.svg` to that tiny worker, which strictly escapes query text
and returns the package, version, and gzip value in SVG. The product analysis
remains fully client-side; the worker only renders a supplied badge label.
