...
function renderResults() {
  ...
  results.innerHTML = `<div class="result-head">
    ...
    <section class="share-desk" aria-labelledby="share-heading"><div><p class="kicker">Pass it on</p><h3 id="share-heading">Share the exact edition.</h3><p>The link reruns this public package and version in the recipient’s browser. Its embeddable SVG badge carries this report’s package, version, and measured gzip figure. Copy Badge also gives you a self-contained safe SVG.</p></div><div class="share-actions"><button id="copy-link" class="secondary-button" type="button">Copy result link</button><button id="copy-badge" class="quiet-button" type="button">Copy measured badge</button><a id="badge-link" class="quiet-button badge-link" href="${escapeHtml(badgeUrl(location.origin, badgeValues()))}" target="_blank" rel="noreferrer">Open SVG badge</a><span id="copy-status" role="status"></span></div></section>
