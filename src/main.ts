...
const routeMeta: Record<RouteName, { title: string; description: string; path: string }> = {
  home: { title: "Package Cost Explorer — Compare npm package costs", description: "Compare installed size and bundle size for npm package entry points before adding a dependency.", path: "/" },
...
function shell(content: string, demo = false): string {
  ...
  return `<div class="paper${demo ? " demo-mode" : ""}">
    ...
    ${content}
    <footer class="footer">
      <p>Compare installed size and bundle size before adding a dependency.</p>
...
function renderHome() {
  ...
  app.innerHTML = shell(`<main id="main"><section class="hero" aria-labelledby="page-title"><div class="hero-copy">
    ...
    <ul class="proof-points" aria-label="Product facts"><li><span aria-hidden="true">01</span> No payment or account.</li><li><span aria-hidden="true">02</span> Reloads offline after the first visit.</li><li><span aria-hidden="true">03</span> Real measurements contact npm directly.</li></ul>
    <form id="search-form" class="search-form" novalidate><label for="package-input">Package and version</label><div class="search-row"><input id="package-input" name="package" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="date-fns@latest" aria-describedby="package-help package-error" required /><button class="secondary-button" type="submit">Measure this package</button></div><p id="package-help" class="field-note">See its installed size and bundle sizes. Try <button type="button" class="inline-example" data-example="date-fns@latest">Use date-fns</button> or <button type="button" class="inline-example" data-example="lodash-es@latest">Use lodash-es</button>.</p><p id="package-error" class="field-error" aria-live="polite"></p></form>
